...
from dataset.dataset import BilingualDataset
import torch
import sys
...
def translate(sentence: str):
    ...
    tokenizer_src = Tokenizer.from_file(
        str(Path(config["tokenizer_file"].format(lang=config["lang_src"])))
    )
    tokenizer_tgt = Tokenizer.from_file(
        str(Path(config["tokenizer_file"].format(lang=config["lang_tgt"])))
    )
    model = Transformer(
